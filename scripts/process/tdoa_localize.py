...
def get_sensor_positions():
    ...
    sensor_eastings = df["easting"].to_list()[0:-1]
    sensor_northings = df["northing"].to_list()[0:-1]
...
def localize_tdoa_data(tdoa_csv_path: str) -> pl.DataFrame:
    ...
    for row in df.iter_rows(named=True):
        ...
        brg_3dvha = calculate_bearing(sensor_eastings[0], sensor_northings[0], easting_m, northing_m)
        brg_vla1 = calculate_bearing(sensor_eastings[1], sensor_northings[1], easting_m, northing_m)
        brg_vla2 = calculate_bearing(sensor_eastings[2], sensor_northings[2], easting_m, northing_m)
